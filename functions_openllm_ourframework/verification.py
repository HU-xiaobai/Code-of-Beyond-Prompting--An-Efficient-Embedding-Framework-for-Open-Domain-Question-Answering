...
def use_api_verif(model, model_type, tokenizer, dataset, choices, passages, all_token, pred_idx):
    queries = []
    answers = []
...
def use_api_rank(model, model_type, tokenizer, dataset, choices, passages1, passages2, all_token):
    res = []
    ...
    for i, example in tqdm(enumerate(dataset)):
        ...
        answer1, use_token1 = api_query(model, model_type, tokenizer, query1,ground_truth=ground_truth)
        all_token+=use_token1
        answer1 = answer1[0]

        answer1 = answer1.replace('.','') 

        query2 = ranking_summary(example, passages2[i], passages1[i])
        answer2, use_token2 = api_query(model, model_type, tokenizer, query2,ground_truth=ground_truth)
        all_token+=use_token2
