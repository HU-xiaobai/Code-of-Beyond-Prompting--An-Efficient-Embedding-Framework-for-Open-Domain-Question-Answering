...
if __name__ == '__main__':
    ...
    if  args.data_name == 'hotpotqa' or "2wiki":
       ...
       def extract_subset(dataset, indices):
           return {
               "question": [dataset[i]["question"] for i in indices],
               "answers": [dataset[i]["answer"] for i in indices],
               "ids": [dataset[i]["_id"] for i in indices],
           }
    ...
    if args.data_name == 'hotpotqa' or '2wiki':
        raw_passages = []
        with open(args.documents_pool, 'r') as f:
            for line in f:
                raw_passages.append(json.loads(line.strip()))
    ...
    if args.retrieval_method == 'bm25':
        model = BM25(hostname=args.elastic_search_server, index_name="hotpotqa_bm25", initialize=False)
